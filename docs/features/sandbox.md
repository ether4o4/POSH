...
- **File visibility**: `/root` lives at app-external storage so files the agent produces can be opened with `open_file` via Android's `FileProvider`. The rest of the filesystem (`/etc`, `/usr`, `/var`, etc.) is the Alpine rootfs and lives in app-internal storage. One consequence: app-external storage is mounted non-executable, so programs placed under `/root` cannot be launched — executables belong on the rootfs side. The local-model engine binary lives there for exactly this reason, which means a sandbox reset deletes it (it is simply re-downloaded on the next engine setup) while downloaded models under `/root` survive the reset.
