...
3. **Run** — starts the server with the model's own chat template (so answers and tool calls are well-formed), a context window of 4096 tokens, and a thread count tuned for phone big.LITTLE CPUs. A RAM preflight fails fast with a plain-language message when the model plus working headroom (KV cache and the app itself) clearly won't fit in free memory — running anyway is how the low-memory killer takes the whole app down mid-chat. A running server **auto-stops after 30 minutes without a request** to save battery and RAM; tapping Run starts it again, and the card says so while serving.
